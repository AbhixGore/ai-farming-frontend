...
function ProfileModal({ onSave, language }) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={rowStyle}>
          ...
          <input style={inputStyle} placeholder="उदा: रामराव पाटील"
        ...
        <div style={rowStyle}>
          ...
          <select style={inputStyle} value={form.district} onChange={e => update("district", e.target.value)}>
            ...
            <optgroup label="मराठवाडा">
              ...
              <option>बीड</option><option>नांदेड</option>
              <option>लातूर</option><option>उस्मानाबाद (धाराशिव)</option>
              <option>परभणी</option><option>हिंगोली</option><option>जालना</option>
            </optgroup>
            <optgroup label="विदर्भ">
              <option>नागपूर</option><option>अमरावती</option>
              <option>अकोला</option><option>यवतमाळ</option>
              <option>वर्धा</option><option>बुलढाणा</option><option>वाशीम</option>
            </optgroup>
            <optgroup label="पश्चिम महाराष्ट्र">
              <option>पुणे</option><option>नाशिक</option>
              <option>सोलापूर</option><option>सातारा</option>
              <option>सांगली</option><option>कोल्हापूर</option><option>अहमदनगर</option>
            </optgroup>
            <optgroup label="कोकण">
              <option>मुंबई</option><option>रत्नागिरी</option>
              <option>रायगड</option><option>सिंधुदुर्ग</option><option>ठाणे</option>
            </optgroup>
          </select>
        </div>
        ...
        <div style={rowStyle}>
          ...
          <input style={inputStyle} placeholder="उदा: गेवराई"
            value={form.taluka} onChange={e => update("taluka", e.target.value)} />
        </div>

        <div style={rowStyle}>
          <label style={labelStyle}>जमीन (एकरात)</label>
          <input style={inputStyle} type="number" placeholder="उदा: 5"
            value={form.landAcres} onChange={e => update("landAcres", e.target.value)} />
        </div>
        ...
        <div style={rowStyle}>
          ...
          <select style={inputStyle} value={form.soilType} onChange={e => update("soilType", e.target.value)}>
            ...
            <option>मध्यम काळी माती</option><option>हलकी काळी माती</option>
            <option>लाल माती</option><option>वालुकामय माती</option><option>मुरमाड माती</option>
          </select>
        </div>
        ...
        <div style={rowStyle}>
          ...
          <input style={inputStyle} placeholder="उदा: सोयाबीन"
            value={form.currentCrop} onChange={e => update("currentCrop", e.target.value)} />
        </div>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
          <button onClick={() => onSave({})} style={{
            ...
            border: "1.5px solid #c8e6c9", background: "white",
            color: "#2e7d32", fontSize: 14, cursor: "pointer",
            fontFamily: "'Segoe UI', Arial, sans-serif"
          }}>नंतर सांगतो</button>
...
export default function App() {
  ...
  const [speakingIndex, setSpeakingIndex] = useState(null); // which message is speaking
  const [farmerProfile, setFarmerProfile] = useState(null);
  const [language, setLanguage] = useState(LANGUAGES[0]); // default Marathi
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setChat([...updatedChat, { role: "ai", text: "⏳ कनेक्शन एरर. पुन्हा प्रयत्न करा.", feedback: null }]);
    }
    setLoading(false);
  };
  ...
  return (
    <div style={{
      ...
      fontFamily: "'Segoe UI', Arial, sans-serif", background: "#f0f4f0"
    }}>

      {farmerProfile === null && <ProfileModal onSave={(p) => setFarmerProfile(p)} language={language} />}
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 6, flexShrink: 0, alignItems: "center" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {showLangPicker && (
              <div style={{
                position: "absolute", top: 36, right: 0, background: "white",
                borderRadius: 12, boxShadow: "0 4px 16px rgba(0,0,0,0.15)",
                zIndex: 100, overflow: "hidden", minWidth: 130
              }}>
                {LANGUAGES.map(lang => (
                  <button key={lang.code} onClick={() => { setLanguage(lang); setShowLangPicker(false); }} style={{
                    display: "flex", alignItems: "center", gap: 8,
                    width: "100%", padding: "10px 14px", border: "none",
                    background: language.code === lang.code ? "#e8f5e9" : "white",
                    color: "#333", fontSize: 14, cursor: "pointer",
                    fontFamily: "'Segoe UI', Arial, sans-serif",
                    borderBottom: "1px solid #f0f0f0"
                  }}>
                    <span>{lang.flag}</span>
                    <span style={{ fontWeight: language.code === lang.code ? "600" : "400" }}>
                      {lang.label}
                    </span>
          ...
          <button onClick={() => setFarmerProfile(null)} title="माहिती बदला" style={{
            background: "rgba(255,255,255,0.15)", border: "none", color: "white",
            padding: "5px 9px", borderRadius: 16, fontSize: 13, cursor: "pointer"
          }}>✏️</button>

          {chat.length > 0 && (
            <button onClick={() => { setChat([]); window.speechSynthesis.cancel(); setSpeakingIndex(null); }} style={{
      ...
      >
        {chat.length === 0 && (
          <div style={{ padding: "8px 4px" }}>
            <div style={{
              textAlign: "center", padding: "24px 16px 20px", background: "white",
              borderRadius: 16, marginBottom: 16, boxShadow: "0 1px 4px rgba(0,0,0,0.08)"
            }}>
              ...
              <p style={{ color: "#999", fontSize: 12, margin: 0 }}>
                🎤 बोलून विचारा किंवा टाइप करा · भाषा: {language.flag} {language.label}
              </p>
            </div>
            <p style={{ fontSize: 13, color: "#888", marginBottom: 10, paddingLeft: 4 }}>💬 असे विचारा:</p>
            {suggestedQuestions.map((q, index) => (
              <button key={index} onClick={() => sendMessage(q.text)} style={{
                display: "flex", alignItems: "center", gap: 10, width: "100%",
                padding: "11px 14px", marginBottom: 8, background: "white",
                border: "1px solid #e8f5e9", borderRadius: 12, fontSize: 14,
        ...
        {chat.map((msg, index) => (
          <div key={index} style={{
            ...
            <div style={{
              ...
              {msg.role === "ai" && (
                <div style={{
                  width: 30, height: 30, borderRadius: "50%", background: "#2e7d32",
                  display: "flex", alignItems: "center", justifyContent: "center",
              ...
              <div style={{
                ...
                boxShadow: "0 1px 4px rgba(0,0,0,0.1)", fontSize: 14,
            ...
            {msg.role === "ai" && (
              <div style={{ display: "flex", gap: 6, marginLeft: 38, marginTop: 2, flexWrap: "wrap" }}>
                ...
                {/* Single speak/stop toggle button */}
                <button onClick={() => handleSpeakToggle(index, msg.text)} style={{
                  background: speakingIndex === index ? "#e8f5e9" : "white",
                  border: speakingIndex === index ? "1px solid #2e7d32" : "1px solid #e0e0e0",
                  borderRadius: 20, padding: "3px 10px", fontSize: 13,
                  cursor: "pointer",
                  color: speakingIndex === index ? "#2e7d32" : "#555",
                  fontFamily: "'Segoe UI', Arial, sans-serif",
                  transition: "all 0.2s"
                }}>
                  {speakingIndex === index ? "⏹ थांबवा" : "🔊 ऐका"}
                </button>
                ...
                {msg.feedback === "up" ? (
                  ...
                  <>
                    <button onClick={() => handleFeedback(index, "up")} style={{
                      ...
                      padding: "3px 10px", fontSize: 13, cursor: "pointer", color: "#555",
                      fontFamily: "'Segoe UI', Arial, sans-serif"
                    }}>👍 बरोबर</button>
        ...
        {loading && (
          <div style={{ display: "flex", alignItems: "flex-end", gap: 8 }}>
            ...
            <div style={{
              ...
              boxShadow: "0 1px 4px rgba(0,0,0,0.1)", color: "#888", fontSize: 14
            }}>विचार करत आहे... 🌱</div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      {/* INPUT BAR */}
      <div style={{
        padding: "10px 12px", background: "white", borderTop: "1px solid #e0e0e0",
        ...
        <input
          ...
          style={{
            ...
            border: "1.5px solid #c8e6c9", fontSize: 14, outline: "none",
            background: loading ? "#f9f9f9" : "white", color: "#333",
            fontFamily: "'Segoe UI', Arial, sans-serif"
          }}
        />
